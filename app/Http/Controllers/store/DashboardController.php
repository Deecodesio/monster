...
class DashboardController extends BaseController
{
...

  {
    ...
    $cod = [
      $monday_cod_earnings, $tuesday_cod_earnings, $wednesday_cod_earnings, $thursday_cod_earnings, $friday_cod_earnings, $saturday_cod_earnings, $sunday_cod_earnings
    ];
    //cod end
    ...
    $online = [
      $monday_online_earnings, $tuesday_online_earnings, $wednesday_online_earnings, $thursday_online_earnings, $friday_online_earnings, $saturday_online_earnings, $sunday_online_earnings
    ];
    ...
    return view('store/index', [
      'total_users' => $total_users, 'total_restaurants' => $total_restaurants, 'total_earnings' => $total_earnings,
  ...
  public function dashboard_statistics(Request $request)
  {
    ...
    $res1 = DB::table('restaurants')->where('id', $request->id)->first();
    $business = DB::table('business_type')->where('id', $res1->business_type)->first();
    ...
    if ($business->layout_id == 2) {
      $today_res_comissions[] = [
        'subtitle' => "Today Store Commission",
    ...
    if ($business->layout_id == 2) {
      ...
      $total_res_comissions[] = [
        'subtitle' => "Total Store Commission",
    ...
    $req = DB::table('requests')->where('restaurant_id',  $request->id)->orderby('created_at', 'asc')->where('status', 7)->select(
      ...
      ->toArray();
    $can = [10, 11, 12, 13, 14];
    $req2 = DB::table('requests')->where('restaurant_id',  $request->id)->orderby('created_at', 'asc')->whereIN('status', $can)->select(
      DB::raw("(COUNT(*)) as count"),
      DB::raw("MONTHNAME(created_at) as month_name")
    )
      ->groupBy('month_name')
    ...
    $recent = DB::table('requests')->where('restaurant_id',  $request->id)->orderby('id', 'desc')->get();
