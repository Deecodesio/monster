...
// dd('NEW PASSWORD IS = '.$request->password.' '.$password);
    ...
    public function today_earnings(Request $request)
    {
        ...
        $response_array = array('status'=>true,'today_earnings'=>$today_earnings,'today_incentives'=>
                            $today_incentives,'tips' => $today_tips);
        $response = response()->json($response_array, 200);
        return $response;
    }
    ...
    public function weekly_earnings(Request $request)
    {
        ...
        $graph_data = $foodrequest->where('delivery_boy_id',$user_id)->where('is_paid',1)
                            ...
                            ->groupBy('day')
                            ->get();
        $response_array = array('status'=>true,'weekly_earnings'=>round($weekly_earnings,2),'weekly_tips'=>round($weekly_tips,2),'weekly_incentives'=>
    ...
    public function monthly_earnings(Request $request)
    {
        ...
        $graph_data = $foodrequest->where('delivery_boy_id',$user_id)->where('is_paid',1)
                            ...
                            ->groupBy('day')
                            ->get();
        $response_array = array('status'=>true,'monthly_earnings'=>round($monthly_earnings,2),'monthly_tips'=>round($tips,2),'monthly_incentives'=>
