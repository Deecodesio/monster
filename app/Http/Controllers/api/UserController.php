...
class UserController extends BaseController
{
    ...
    public function get_banners(Request $request)
    {
        ...
        if ($request->business_id == 0) {
            $res = DB::table('restaurants')->where('status', 1)->get();
            $RestIDs_all = [];
            foreach ($res as $restaurant) {
                $distance = vincentyGreatCircleDistance($source_lat, $source_lng, $restaurant->lat, $restaurant->lng);
                if ($distance <= DEFAULT_RADIUS) {
                    $RestIDs_all[] = $restaurant->id;
                }
            }
            $data = DB::table('offers_banner')->whereIn('restaurant_id', $RestIDs_all)->where('status', 1)->where('featured', 1)->orderBy('position', 'asc')->get();
        } else {
            $RestIDs = [];
            $res = DB::table('restaurants')->where('business_type', $request->business_id)->where('status', 1)->get();
            foreach ($res as $restaurant) {
                $distance = vincentyGreatCircleDistance($source_lat, $source_lng, $restaurant->lat, $restaurant->lng);
                if ($distance <= DEFAULT_RADIUS) {
                    $RestIDs[] = $restaurant->id;
                }
            }
            $data = DB::table('offers_banner')->whereIn('restaurant_id', $RestIDs)->where('status', 1)->orderBy('position', 'asc')->get();
        }
        foreach ($data as $k => $d) {
            $data[$k]->is_open = restaurant_is_open($d->restaurant_id);
        }
        foreach ($data as $datas) {
            $rest_d = DB::table('restaurants')->where('id', $datas->restaurant_id)->first();
            if ($rest_d) {
                $layout = DB::table('business_type')->where('id', $rest_d->business_type)->first();
                $datas->layout_id = $layout->layout_id;
                $datas->restaurant_name = $rest_d->restaurant_name;
            }
        }

        foreach ($data as $datas) {
            $datas->image = BASE_URL . UPLOADS_PATH . $datas->image;
            if ($datas->category != null && $datas->category != 0) {
                $cat_det = DB::table('business_category')->where('id', $datas->category)->first();
                $datas->redirect_to = $datas->category;
                $datas->redirect_type = 1;
                $datas->redirect_name = $cat_det->category_name;
            } else if ($datas->product && $datas->product != null && $datas->product != 0) {
                $product = DB::table('food_list')->where('id', $datas->product)->first();
                $datas->redirect_to = $datas->product;
                $datas->redirect_type = 2;
                $datas->redirect_name = $product->name;
            } else {
                $datas->redirect_to = 0;
                $datas->redirect_type = 0;
                $datas->redirect_name = "";
            }
        }
        if (count($data) != 0) {
    ...
    public function get_nearby_restaurant(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($data) {
                ...
                $response_array = array('status' => false, 'schedule' => $schedule, 'message' => 'No Stores Found');
            }
        }
    ...
    public function get_favourite_list(Request $request)
    {
        ...
        if (count($data) != 0) {
            ...
            $response_array = array('status' => false, 'message' => 'No favourite stores found');
        }
    ...
    public function track_order_detail(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $layout_details = DB::table('business_type')->where('id', $restaurant_detail->business_type)->first();

            if (!$restaurant_detail) {
                return response()->json(['status' => false, 'error_code' => 101, 'message' => 'Store not found'], 200);
            }
            ...
            $order_status[] = array(
                ...
                'layout_id' => $layout_details->layout_id,
            );
    ...
    public function content(Request $request, $type)
    {
        ...
        // if($type == 'faq')
        // {
        //     $content = fopen(Storage::path("faq.rtf"), 'r');
        //     while (!feof($content)) {
    ...
    public function get_near_restaurant(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($business_id == 2) {
                //geofencing for restaurant
                $data = $restaurants->where('status', 1)
                    ->select('restaurants.*')
    ...
    public function get_near_restaurant1(Request $request)
    {
    ...
    public function get_near_brands(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $nearbyRestaurantIDs = Restaurant::nearbrandDistanceBusiness($source_lat, $source_lng, $business_id);
    ...
    public function get_brand_rest(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $nearbyRestaurantIDs = Restaurant::nearbrandDistance($source_lat, $source_lng, $business_id);
    ...
    public function get_more_restaurant(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $nearbyRestaurantIDs = Restaurant::moreDistance_business($source_lat, $source_lng, $business_id);
            //geofencing for restaurant
            /*    $data = $restaurants->with('Cuisines')->where('status', 1)
                ->select('restaurants.*')
                ->whereIn('id', $nearbyRestaurantIDs)
            ...
            foreach ($nearbyRestaurantIDs as $res) { //print_R($business_id);
                $data = $restaurants->with('Cuisines')->where('status', 1)
                    ->select('restaurants.*')
                    ->where('id', $res->id)
                    ->orderBy('is_open')
    ...
    public function get_more_store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $nearbyRestaurantIDs = Restaurant::moreDistance_business_new($source_lat, $source_lng, $business_id);
            //geofencing for restaurant
            /*    $data = $restaurants->with('Cuisines')->where('status', 1)
                ->select('restaurants.*')
                ->whereIn('id', $nearbyRestaurantIDs)
    ...
    public function get_more_restaurant1(Request $request)
    {
    ...
    {
        $types = DB::table('business_type')->where('status', 1)->get();
        ...
        $response_array = array('status' => true, 'business_types' => $types);
        $response = response()->json($response_array, 200);
        return $response;
    }


    public function package_type(Request $request)
    {
        $types = DB::table('package_type')->get();

        $response_array = array('status' => true, 'package_type' => $types);
        $response = response()->json($response_array, 200);
        return $response;
    }
    public function topupWallet(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $user_detail = $this->users->find($request->user_id);
            ...
            $total = $user_detail->wallet_amount +  $amount;
            DB::table('user_wallet_history')->insert($status_entry);

            $this->users->find($request->user_id)->increment('wallet_amount', $amount);
            $user_noti = DB::table('users')->where('id', $request->user_id)->first();
            $title = "Wallet";
            $message = 'Your wallet is topped up with amount '  . $amount;
            sendNotification([$user_noti->device_token], $title, $message, ['amount' => $amount]);
        }
        $response_array = array('status' => true, 'message' => $message, 'total' => $total);
        $response = response()->json($response_array, 200);
        return $response;
    }
    ...
    public function payment_details(Request $request)
    {
        ...
        if ($pg->value == 1) {
            ...
            // $juspay = generate_juspay_session(001, $user_detail, $amount);
            // $js = json_decode($juspay);
            // $sdk_payload = $js->sdk_payload;
            $response_array = [
                'status' => true,
                'PaymentID' => "",
                'ClientSecret' =>  "",
    ...
    public function walletbalance(Request $request)
    {
        ...
        $response_array = array('status' => true, 'user' => $user);
        $response = response()->json($response_array, 200);
        return $response;
    }
    public function edit_delivery_address(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $check_for_address = DB::table('delivery_address')->where('id', $address_id)->first();

            if (!$check_for_address) {
                $response_array = array('status' => false, 'message' => 'Address Not Found');
            } else {
                $data = DB::table('delivery_address')->where('id', $address_id)->update([
                    'address' => $address,
                    'lat' => $lat,
                    'lng' => $lng,
                    'type' => $type,
                ...
                $return_address = DB::table('delivery_address')->where('id', $address_id)->first();
                $response_array = array('status' => true, 'data' => $return_address, 'message' => 'Address Edited successfully');
            }
        }
        $response = response()->json($response_array, 200);
        return $response;
    ...
    public function get_nearby_promo_list(Request $request)
    {
        if ($request->header('authId') != "") {
            $user_id = $request->header('authId');
        } else {
        ...
        $nearbyRestaurantIDs = Restaurant::nearbyRestaurantIDs($source_lat, $source_lng);


        $get_promo = $this->promocode->where('coupon_code.status', 1)
            //    ->whereIn('restaurant_id', $nearbyRestaurantIDs)
            //    ->Orwhere('restaurant_id', null)
            ->leftjoin('restaurants', 'restaurants.id', '=', 'coupon_code.restaurant_id')
            ->select('coupon_code.*', 'restaurants.image')
            ->whereDate('available_from', '<=', Carbon::now())
        ...
        foreach ($get_promo as $key => $each_promo) {
            // checking menu avaible now

            $available = 0;
            $coupon_time = DB::table('coupon_timing')->where('coupon_id', $each_promo->id)->get();

            if (sizeof($coupon_time)) {

                foreach ($coupon_time as $key => $time) {
                    if ($available != 1) {
            ...
            if ($available == 1) {
                ...
                $promocode_list_restaurant[] = $each_promo;
            }
        }
        $response_array = array('status' => true, 'promo_list' => $promocode_list_restaurant);
        $response = response()->json($response_array, 200);
        return $response;
    }
    public function get_defaults(Request $request)
    {
        $disclaimer_enable = DB::table("settings")->where('key_word', 'disclaimer_enable')->first();
        $disclaimer = DB::table("settings")->where('key_word', 'disclaimer')->first();

        $track_page = DB::table("settings")->where('key_word', 'tracking_page')->first();



        $rest = DB::table('restaurants')->first();

        $response_array = array('status' => true, 'disclaimer_enable' => (int)$disclaimer_enable->value, 'disclaimer' => $disclaimer->value, 'track_page' => (int)$track_page->value, 'restaurant_id' => (string) $rest->id);
        $response = response()->json($response_array, 200);
        return $response;
    }
        ...
        if (count($delivery_charge_limits)) {
            foreach ($delivery_charge_limits as $dcl) {
                ...
                if ($deliver_charge_mode == 1) {
                    ...
                    if ($total_price >= $dcl->Minimum && ($dcl->Maximum == null || $total_price <= $dcl->Maximum)) {
