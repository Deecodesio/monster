...
class BaseController extends Controller
{
    ...
    public static function send_otp($NUMBER, $OTP)
    {
        ...
        $auth = DB::table('settings')->where('key_word', 'msg91_auth_key')->first();
        $template = DB::table('settings')->where('key_word', 'msg91_template_id')->first();

        $curl = curl_init();

        curl_setopt_array($curl, [
            CURLOPT_URL => "https://api.msg91.com/api/v5/otp?template_id=" . $template->value . "&mobile=" . $NUMBER . "&authkey=" . $auth->value . "&otp_length=5",
    ...
    public static function verify_otp($phone, $otp)
    {
        ...
        curl_setopt_array($curl, [
            CURLOPT_URL => "https://api.msg91.com/api/v5/otp/verify?otp=" . $otp . "&authkey=" . $auth->value . "&mobile=" . $phone . "",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
    ...
    public function getDynamicLocale1($obj, $key)
    {
        ...
        if ($obj->{$key}) {
            $obj->{$key} =  html_entity_decode($obj->{$key});
        }
    ...
    public function updateSecondaryLangAddon($addons, $request)
    {
        foreach ($addons as $inx => $addon) {
            ...
            foreach ($keys as $key) {
                if (Str::startsWith($key, 'secondary_')) {
                    $original_key = Str::substr($key, strlen('secondary_'));
                    $addons[$inx]->{$original_key} = $this->getDynamicLocale1($addon, $original_key, $request);
                }
            }
        }
        return $addons;
    ...
    public function updateSecondaryLang_web($obj, $request)
    {
            ...
            foreach ($items->items as  $ind => $item) {
                ...
                foreach ($keys as $key) {
                    ...
                    if (Str::startsWith($key, 'secondary_')) {
                        ...
                        $json_obj[$index]->items[$ind]->{$original_key} = $this->getDynamicLocale1($item, $original_key, $request);
                        $json_obj[$index]->items[$ind]->add_ons = $this->updateSecondaryLangAddon($item->add_ons, $request);
                        $json_obj[$index]->items[$ind]->food_quantity = $this->updateSecondaryLangAddon($item->food_quantity, $request);
                    }
                }
    ...
    public function secondLanguage($name, $second)
    {

        $locale = session('locale_admin');
    ...
    public function secound_lang_name($name)
    {

        $value = mb_convert_encoding($name, 'HTML-ENTITIES', 'UTF-8');

        return $value;
    }
    public function secound_name($name)
    {
    ...
    public function secound_name_api($name)
    {

        $value = html_entity_decode($name, ENT_QUOTES | ENT_XML1, 'UTF-8');


        return $value;
    }

    public function secondLanguage_store($name, $second)
    {

        $locale = session('locale_store');
    ...
    public function secondLanguage_user($name, $second)
    {
