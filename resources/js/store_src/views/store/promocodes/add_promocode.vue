      ...
      <b-row>
        ...
        <b-col md="6" v-if="!isHidden_lang">
          <b-form-group :label="$t('titles') + '*'">
            <validation-provider #default="{ errors }" :name="$t('titles')"
              :custom-messages="{ required: $t('The Titles field is required') }" rules="required">


              <b-form-input id="mc-last-name" :placeholder="$t('titles')" v-model="promocode.title" />
        ...
        <b-col md="6" v-if="!isHidden_lang">
          <b-form-group :label="$t('promocode') + '*'">
            <validation-provider #default="{ errors }" name="PromoCode"
        ...
        <b-col md="6">
          <b-form-group :label="$t('maximum') + ' ' + $t('amount') + '*'">
            <validation-provider #default="{ errors }" name="Maximum_amount" rules="required">
              <b-form-input id="Maximum_amount" :placeholder="$t('maximum') + ' ' + $t('amount')"
        ...
        <b-col md="6">
          <b-form-group :label="$t('minimum') + ' ' + $t('order') + ' ' + $t('value') + '*'">
            <validation-provider #default="{ errors }" name="Minimum Order value" rules="required">
              <b-form-input id="Minimum_Order_value" :placeholder="$t('minimum') + ' ' + $t('order') + ' ' + $t('value')"
        ...
        <b-col md="6">
          <b-form-group :label="$t('discount') + '(%) *'">
            <validation-provider #default="{ errors }" name="Discount" rules="required">
              <b-form-input id="mc-last-name" placeholder="Enter Discount" v-model="promocode.discount" />
              <small class="text-danger">{{ errors[0] }}</small>
            </validation-provider>
          </b-form-group>
        </b-col>
        <b-col md="6">
          <b-form-group :label="$t('usage') + ' ' + $t('limit') + ' ' + $t('per') + ' ' + $t('coupon') + '*'">
            <validation-provider #default="{ errors }" name="Usage Limit Per Coupon" rules="required">
        ...
        <b-col md="6">
          <b-form-group :label="$t('usage') + ' ' + $t('limit') + ' ' + $t('per') + ' ' + $t('User') + '*'">
            <validation-provider #default="{ errors }" name="Usage Limit Per User" rules="required">
        ...
        <b-col md="6">
          <b-form-group :label="$t('available') + ' ' + $t('from') + '*'">
            <validation-provider #default="{ errors }" name="Available From" rules="required">
        ...
        <b-col md="6" style="display: none;"></b-col>
        <b-col md="6">
...
import {
  BImg, BMedia, BRow, BCol, BFormSelect, BFormRadioGroup, BFormDatepicker, BFormGroup,
  BFormInput, BFormCheckbox, BForm, BButton, BCard, BFormTimepicker, BTabs, BTab,
} from 'bootstrap-vue'
...
export default {
  components: {
    ...
    ValidationObserver,
    vSelect,
    BImg, BMedia, BCard, BFormTimepicker, BTabs, BTab,
  },
  directives: {
    Ripple,
  },
  ...
  methods: {
    ...
    promocode_add() {
      ...
      this.$http
        .post("/store/add_to_promocode", promo)
        .then(
          (response => {
  ...
  created() {
    ...
    if (this.$route.params.id) {
      this.$http.get('/store/edit_promocode/' + this.$route.params.id)
        .then(res => {
          this.promocode = res.data.coupon;
          this.promocode.discount = this.promocode.value;
