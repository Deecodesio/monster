<template>
        ...
        <b-row>
          <b-col md="6">
            <div class="custom-search d-flex justify-content-start">
              <b-button variant="primary" :to="{ name: 'add_category' }">
                <i class="fa-solid fa-plus"></i> {{ $t('Add Sub Category') }}
              </b-button>
            </div>
  ...
  perPage: pageLength
          ...
          <template slot="table-row" slot-scope="props">
            ...
            <!-- Column: Name -->
            <div v-if="props.column.field === 'category_images'" class="text-nowrap">
              <b-img :src="props.row.image" rounded fluid id="category-img" alt="category photo" />
            </div>
            ...
            <span v-else-if="props.column.field === 'status'">
              <div v-if="props.row.restaurant_id != null">
            ...
            <span v-else-if="props.column.field === 'action'">
              <div v-if="props.row.restaurant_id != null">
                <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                  cursor="pointer" @click="$router.push({ name: 'edit_category', params: { id: props.row.id } })" />
                <b-tooltip placement="left" :title="$t('edit category')"
                ...
                <feather-icon :id="`invoice-row-delete-preview-icon`" icon="Trash2Icon" size="16" class="mx-1"
                  cursor="pointer" @click="showMsgBoxTwo(props.row.id, props.row.category_name)" />
                <b-tooltip placement="left" :title="$t('delete category')" :target="`invoice-row-delete-preview-icon`" />
              </div>
...
import {
  BImg, BMedia, BAvatar, BBadge, BPagination, BFormGroup, BFormInput, BFormSelect, BDropdown, BButton,
  BDropdownItem, BCard, BTooltip,
} from 'bootstrap-vue'
...
export default {
  ...
  created() {
    ...
    this.$http.get('/store/category_list/' + localStorage.id)
      .then(res => {
        this.rows = res.data;
  ...
  methods: {
    ...
    showMsgBoxTwo(id, res_name) {
      ...
      this.$bvModal
        ...
        .then(value => {
          if (value == true) {
            this.$http.get('/store/delete_category/' + id)
              .then(res => {
    ...
    changedefault(id) {
      this.$http.get('/store/category_status/' + id)
        .then(res => {
