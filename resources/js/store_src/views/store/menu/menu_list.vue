        ...
        <b-row>
          <b-col md="6">
            <div class="custom-search d-flex justify-content-start">
              <b-button variant="primary" :to="{ name: 'add_menu' }">
                <i class="fa-solid fa-plus"></i>{{ $t('add') }} {{ $t('menu') }}
              </b-button>
            </div>
          ...
          <b-badge :variant="statusVariant(props.row.status)">
            ...
            <span v-else-if="props.column.field === 'action'">
              <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                cursor="pointer" @click="$router.push({ name: 'edit_menu', params: { id: props.row.id } })" />
              <b-tooltip placement="left" :title="$t('edit') + ' ' + $t('menu')" :target="`invoice-row-${props.row.id}-preview-icon`" />

              <feather-icon :id="`invoice-row-delete-preview-icon`" icon="Trash2Icon" size="16" class="mx-1"
                cursor="pointer" @click="showMsgBoxTwo(props.row.id, props.row.menu_name)" />
              <b-tooltip placement="left" :title="$t('delete') + ' ' + $t('menu')" :target="`invoice-row-delete-preview-icon`" />
            </span>
...
import {
  BImg, BMedia, BAvatar, BBadge, BPagination, BFormGroup, BFormInput, BFormSelect, BDropdown, BDropdownItem, BCard, BTooltip, BButton, 
} from 'bootstrap-vue'
  ...
  methods: {
    ...
    showMsgBoxTwo(id, res_name) {
      ...
      this.$bvModal
        ...
        .then(value => {
          if (value == true) {
            this.$http.post('/store/delete_menu/' + id)
              .then(res => {
