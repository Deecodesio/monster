        ...
        <b-row>
            ...
            <b-col lg="4" md="4" cols="12">
                ...
                <div class="text-r side sticky-top3 dsp">
                    ...
                    <div id="bill" class="pt-1 cart-bord" style="margin-bottom: 20px">
...
export default {
    ...
    methods: {
      ...
      this.isLoading=true
      if(refName == 0){
        this.$http.get('/store/get_food_list_by_menu/' +this.rest_id +"/" +this.$route.params.id)
        .then(res => {
          this.rows_detail = res.data.items
    ...
    renderSuggestion(suggestion) {
      ...
      return false
        ...
        revice(){
            ...
            this.$http.post('/store/revice',data)
                .then(res => {
                   if(res.data.status == true){
                    this.$toast({
                            ...
                            this.$router.push({ name: 'view_order', params: { id: this.$route.params.id } });
                   }
                })
    ...
    created() {
        ...
        this.$http
            .get("/store/fetchorder/" + this.$route.params.id)
            .then((res) => {
                console.log(res)
                
                this.order_details = res.data.data[0]
                for(let i=0;i<res.data.items.length;i++){
                  
                    let cart = (JSON.parse(localStorage.getItem("store_cart") ) || [])
                   
                    let ind = res.data.items[i];
                    cart.push({
                        id: ind.id,
                        name: ind.name,
                        isveg: ind.isveg,
