        ...
        <b-row>
            <b-col lg="2" md="2" cols="12" style="background-color: white;box-shadow: 2px 2px 8px rgba(0, 0, 0, 0.2);">
                <div class="text-r side sticky-top3 dsp text-black" style="color: black;">
                    ...
                    <div v-for="(data2, index) in rows_detail2" :key="index"
                        @click="scrollMeTo(data2.menu_id, rows[0].id)" :id="data2.menu_name"
                    ...
                    <b-row>
                        <b-col cols="12" v-for="(data2, index2) in rows_detail" :key="index2" id="products"
                            ...
                            <div :id="data2.menu_name">
                                <b-row>
                                    <b-col xxl="4" lg="6" md="12" cols="12" class="col-xxl-4"
            ...
            <b-col lg="4" md="4" cols="12">
                ...
                <vue-autosuggest v-model="query1" :suggestions="suggestions1" :input-props="inputProps1"
                    :section-configs="sectionConfigs1" :render-suggestion="renderSuggestion1" :limit="10"
                    @input="fetchResults1" />

                <div style="border:2px solid #e6e6e6; padding: 10px; margin:20px 0px; border-radius: 10px;cursor: pointer;"
                    @click="showModal">
                    <i class="fa-solid fa-circle-plus" style="font-size: 12px;"></i> <span style="font-size: 12px;">{{
                        $t("Add") }} {{ $t("New") }} {{ $t("User") }}</span>
                </div>
                ...
                <b-card class="rounded3">
                    <div class="text-r side sticky-top3 dsp">
                                ...
                                <b-col cols="12" style="padding: 1.25rem">
                                    <b-media vertical-align="top">
                                        <template #aside v-if="data.food_image">
                                            <b-img class="bor" :src="data.food_image" blank-color="#ccc" width="40" />
                                        </template>
                                        <h5 class="media-heading" style="height: 40px;">
                                            <div v-if="data.isveg">
                                            ...
                                            <div v-else>
                                                ...
                                                {{ data.name }}
                                                <div class="" @click="open_image(data2, data2)"
                                                    style="color: black; font-weight: 600; font-family: Quicksand; cursor: pointer;padding-top: 4px;">
                                                    {{ data2.label }}
                                                </div>
                                            </div>
                        ...
                        <div id="order_type" class="" style="margin-bottom: 20px" v-if="business_type == 1">
                            <h4>{{ $t("Order Type") }}</h4>
                            <label class="btn btn-outline-secondary tiplabel tiphide  deli f-size-0 tipsactive1"
                                id="dineinp" type="button">
                                <input type="radio" class="checktips" hidden name="d_type" id="delivery"
                                    @change="o_type_sel(1)" value="1" checked><span style="color:#282828;"><i
                                        class="fa-solid fa-utensils"></i> {{ $t("Dine In") }}</span></label>


                            <label class="btn btn-outline-secondary tiplabel tiphide take f-size-0 " id="takeawayp"
                                type="button"> <input type="radio" class="checktips" hidden name="d_type" id="takeaway"
                                    @change="o_type_sel(2)" value="2"><span style="color:#282828;"><i
                                        class="fa-solid fa-person-walking"></i> {{ $t("Take Away") }}</span></label>
                        </div>

                        <div id="payment_type" class="cart-bord" style="margin-bottom: 20px">
                            <h4>{{ $t("Payment Type") }}</h4>
                            <label class="btn btn-outline-secondary tiplabel tiphide  deli f-size-0 tipsactive1"
                                id="cashp" type="button">
                                <input type="radio" class="checktips" hidden name="d_type" id="delivery"
                                    @change="payment_type_sel(1)" value="1" checked><span style="color:#282828;"><i
            ...
            <div id="addonsModalBody" class="text-center">
                <table v-if="add_ons.length" width="80%">
                    ...
                    <tr v-for="(data4, index4) in add_ons" :key="index4">
                        ...
                        <td width="20%" ALIGN="center">
                            <input type="checkbox" :name="'addonsOfItem_' + itemid"
                                :id="'addon_' + itemid + '_' + data4.id" :data-name="data4.name"
                ...
                <table v-if="food_quantity.length" width="80%" style="margin-top:30px;">
                    ...
                    <tr v-for="(data5, index5) in food_quantity" :key="index5">
                        ...
                        <td>
                            <label :for="'size_' + itemid + '_' + data5.id" style="font-size:16px;"> {{ data5.name }}
                            </label>

                        </td>
                        <td>
                            <label :for="'size_' + itemid + '_' + data5.id" style="font-size:16px;"> {{ setting.value }}
                                {{ data5.price }} </label>
                        </td>
                    </tr>
                </table>

                <b-button class="mt-2" variant="primary" id="addCustomizedProduct"
                    @click="addCustomizedProduct(selected)" style="width:100%;">{{ $t("Add") }}</b-button>
            </div>


        </b-modal>

        <b-modal id="modal-login" cancel-variant="outline-secondary" ok-title="Login" cancel-title="Close" centered
            title="Add Customer" hide-footer ref="my-modal">
            <validation-observer ref="simpleRules">
                <b-form>
                    <b-col md="12">
                        <b-form-group>
                            ...
                            <validation-provider #default="{ errors }" rules="required" name="First Name">
                                <b-form-input v-model="cust.name" :state="errors.length > 0 ? false : null"
                                    placeholder="Name" />
                    ...
                    <b-col md="12">
                        <b-form-group>
                            ...
                            <validation-provider #default="{ errors }" rules="required|integer" name="Number">
                                <b-form-input v-model="cust.phone" :state="errors.length > 0 ? false : null"
                    ...
                    <b-col md="12">
                        <b-form-group>
                            ...
                            <validation-provider #default="{ errors }" name="Email" rules="required|email">
                                <b-form-input v-model="cust.email" :state="errors.length > 0 ? false : null"
                                    placeholder="Email" />
                                <small class="text-danger">{{ errors[0] }}</small>
                            </validation-provider>
                        </b-form-group>
                    ...
                    <b-col cols="12">
                        <b-button variant="primary" type="submit" @click.prevent="add_customer">
                            Submit
                        </b-button>
                    </b-col>
                </b-form>
