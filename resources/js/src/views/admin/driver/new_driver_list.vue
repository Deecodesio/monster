  ...
  perPage: pageLength
          ...
          <template slot="table-row" slot-scope="props">
            ...
            <div v-if="props.column.field === 'profile_pics'" class="text-nowrap">
              <b-img :src="props.row.profile_pic" rounded fluid id="new_drive-img" alt="profile photo" />
          ...
          <b-badge :variant="statusVariant(props.row.status)">
            ...
            <span v-else-if="props.column.field === 'action'">
              <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                cursor="pointer" @click="$router.push({ name: 'edit-driver', params: { id: props.row.id } })" />
              <b-tooltip :title="$t('edit') + ' ' + $t('driver')"
              ...
              <feather-icon :id="`invoice-row-delete-preview-icon`" icon="Trash2Icon" size="16" class="mx-1"
                cursor="pointer" @click="showMsgBoxTwo(props.row.id, props.row.partner_id)" />
              <b-tooltip  :title="$t('delete') + ' ' + $t('driver')" :target="`invoice-row-delete-preview-icon`" placement="left"/>
            </span>
...
import {
  BImg, BMedia, BAvatar, BButton, BBadge, BPagination, BFormGroup, BFormInput, BFormSelect, BDropdown, BDropdownItem, BCard, BTooltip,
} from 'bootstrap-vue'
...
export default {
  ...
  created() {
    ...
    this.$http.get('/admin/deliverypeople_list')
      .then(res => {
        this.rows = res.data.message;
        this.Loading = false;
      })
  },
  methods: {
    showMsgBoxTwo(id, res_name) {
      ...
      this.$bvModal
        ...
        .then(value => {
          if (value == true) {
            this.$http.post('/admin/delete_delivery_partner', this.del)
              .then(res => {
                ...
                if (res.data.status == true) {
                  ...
                  this.$http.get('/admin/deliverypeople_list')
                    .then(res => { this.rows = res.data.message })
                } else {
                  this.$toast({
                    component: ToastificationContent,
                    position: 'bottom-right',
