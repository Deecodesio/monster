    ...
    <b-col cols="12">
      <div>
        <form-wizard color="#7367F0" :title="null" :subtitle="null" shape="square" finish-button-text="Submit"
          ...
          <!-- accoint details tab -->
          <tab-content :title="$t('driver') + ' ' + $t('details')" :before-change="validationForm">
        ...
        <span class="d-none d-sm-inline">{{ setting.secondary.value }}</span>
                ...
                <b-col md="4" v-if="isHidden">
                  <b-form-group :label="$t('driver') + ' ' + $t('name') + '*'">
                      ...
                      <b-form-input id="name" type="text" v-model="driver.second_name"
                        :placeholder="$t('name')" />
                  </b-form-group>
                </b-col>
                <b-col md="4">
          ...
          <tab-content :title="$t('document') + ' ' + $t('settings')" :before-change="validationFormDocument">
            <validation-observer ref="doc" tag="form">
              ...
              <div v-for="item in document_view">
                <b-row v-if="item.document_for == 1">
                  ...
                  <b-col md="6">
                    ...
                    <b-form-group >
                      <b-img rounded fluid class="document-img" :id="'document_img' + item.id" src="/no_image.png" alt="category photo" />
                    </b-form-group>
                  </b-col>
...
import {
  ...
  BImg, BMedia, BFormRadio,BFormRadioGroup, BFormCheckboxGroup,
    BFormCheckbox,BModal, VBModal, BTabs, BTab,
  BDropdownForm,
    BDropdownDivider,
    BDropdownText,
...
export default {
  components: {
    ...
    BFormCheckbox,
    BImg, BMedia,BFormRadio,BFormRadioGroup,VuePhoneNumberInput,
    BModal, VBModal, BTabs, BTab,
  BDropdownForm,
    BDropdownDivider,
    BDropdownText,
      BListGroup, BListGroupItem,
  ...
  created() {
    ...
    this.$http.get('/admin/area_list2')
      .then(res => {
...
this.$http.post('/admin/area_list1', this.driver)
  .then(res => {
      ...
      return new Promise((resolve, reject) => {
        this.$refs.accountRules.validate().then(success => {
          if (success) {
            ...
            data.append('second_driver_name', this.driver.second_name ? this.driver.second_name : '');
    ...
    validationFormDocument() {
      return new Promise((resolve, reject) => {
        this.$refs.doc.validate().then(success => {
          if (success) {
            let data = new FormData();
