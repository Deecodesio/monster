  ...
  perPage: pageLength
        ...
        <span v-else>
          ...
          <b-badge :variant="statusVariant(props.row.status)">
            ...
            <span v-else-if="props.column.field === 'status'">
              <span v-if="props.row.order_status === 0">
              ...
              <span v-else-if="props.row.order_status === 1">
              ...
              <span v-else-if="props.row.order_status === 2">

                <b-badge :variant="statusVariant(1)">
                  {{ $t('delivery') }} {{ $t('boy') }} {{ $t('assigned') }}
                </b-badge>

              </span>
              <span v-else-if="props.row.order_status === 3">

                <b-badge :variant="statusVariant(1)">
                  {{ $t('food') }} {{ $t('delivered') }} {{ $t('to') }} {{ $t('delivery') }} {{ $t('boy') }}
                </b-badge>

              </span>
              <span v-else-if="props.row.order_status === 4">

                <b-badge :variant="statusVariant(1)">
                  {{ $t('towards') }} {{ $t('customer') }}
                </b-badge>

              </span>
              <span v-else-if="props.row.order_status === 5">

                <b-badge :variant="statusVariant(1)">
                  {{ $t('reached') }} {{ $t('customer') }}
                </b-badge>

              </span>
              <span v-else-if="props.row.order_status === 6">

                <b-badge :variant="statusVariant(1)">
                  {{ $t('delivered') }} {{ $t('to') }} {{ $t('customer') }}
                </b-badge>

              </span>
              <span v-else-if="props.row.order_status === 7">
...
export default {
  ...
  mail.append('email', localStorage.admin_email);
    this.$http.post('/admin/restaurant_report',mail)
      .then(res => {
    ...
    filterdate()
{
  ...
  if(this.From_Date)
  {
    ...
    export_download()
    {
      ...
      this.$http.post('/admin/order_csv',data)
   .then(res => {
     ...
     window.setInterval(() => {
      ...
      this.$http.post('/admin/order_pdf',data)
   .then(res => {
     ...
     window.setInterval(() => {
      ...
      data.append('from_date', this.From_Date);
      data.append('to_date', this.To_Date);
      this.$http.post('/admin/order_excel',data)
   .then(res => {
