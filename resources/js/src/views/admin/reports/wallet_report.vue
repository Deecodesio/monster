<template>
  ...
  perPage: pageLength
          ...
          <template slot="table-row" slot-scope="props">
            ...
            <!-- Column: Name -->
            <div v-if="props.column.field === 'wallet_amounts'" class="text-nowrap">

              <span class="text-nowrap">{{ setting.value }} {{ props.row.wallet_amount }}</span>
            </div>
          ...
          <b-badge :variant="statusVariant(props.row.status)">
            ...
            <span v-else-if="props.column.field === 'action'">
              <feather-icon :id="'User' + props.row.id" icon="EyeIcon" size="16" class="mx-1"
                @click="history(props.row.id)" cursor="pointer" />
              <b-tooltip :title="$t('User') + ' ' + $t('history')" :target="'User' + props.row.id" />
            </span>
      ...
      <!-- modal backdrop -->
      <b-modal id="modal-no-backdrop" ok-only scrollable content-class="shadow" :title="$t('wallet') + ' ' + $t('history')"
        ok-title="Close" ref="my-modal">
        <b-card-text>
          <table class="table table-striped table-responsive table-bordered">
            ...
            <tbody id="tbody">
              <tr v-for="(w, index) in wallet" :key="index">
                <th>{{ index + 1 }}</th>
                <td>{{ w.order_id }}</td>
                <td>{{ w.amount }}</td>
                <td v-if="w.type === 1"> {{ $t('credit') }}</td>
                <td v-else>{{ $t('debit') }}</td>
...
import {
  ...
  BFormSelect, BDropdown, BDropdownItem, BCard, VBTooltip, BModal, VBModal,BButton,BFormDatepicker,
} from 'bootstrap-vue'
...
export default {
  components: {
    ...
    BDropdownItem,
    BCard,BButton,
    BImg, BMedia, VBTooltip, BModal, VBModal,BFormDatepicker,
  },
  data() {
    return {
      pageLength: 10,
  ...
  created() {
    ...
    this.$http.get('/admin/wallet_report')
      .then(res => {
        this.rows = res.data;
        this.Loading = false;
      })
  ...
  methods: {
    ...
    history(id) {
      ...
      this.$http.get('/admin/fetchwallet/' + id)
        .then(res => {
    ...
    filterdate()
{
  ...
  if(this.From_Date)
  {
    ...
    export_download()
    {
      ...
      data.append('from_date', this.From_Date);
      data.append('to_date', this.To_Date);
      this.$http.post('/admin/wallet_csv',data)
   .then(res => {
     ...
     window.setInterval(() => {
      ...
      this.$http.post('/admin/wallet_pdf',data)
   .then(res => {
     ...
     window.setInterval(() => {
      ...
      data.append('from_date', this.From_Date);
      data.append('to_date', this.To_Date);
      this.$http.post('/admin/wallet_excel',data)
   .then(res => {
