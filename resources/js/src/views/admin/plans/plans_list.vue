<template>
          ...
          <b-row>
            <b-col md="6">
              <div class="custom-search d-flex justify-content-start">
                <b-button variant="primary" :to="{ name: 'add_plans' }">
                  <i class="fa-solid fa-plus"></i> {{ $t('Add plan') }}
                </b-button>
              </div>
            ...
            <template slot="table-row" slot-scope="props">
              ...
              <span v-else-if="props.column.field === 'action'">
                <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                  cursor="pointer" @click="$router.push({ name: 'edit_plans', params: { id: props.row.id } })" />
                <b-tooltip :title="$t('edit plan')" :target="`invoice-row-${props.row.id}-preview-icon`" />
                ...
                <b-tooltip :title="$t('delete plan')" :target="`invoice-row-delete-preview-icon`" />
  ...
  export default {
    ...
    created() {
      ...
      this.$http.get('/admin/plan_list')
        .then(res => {
          this.rows = res.data.plans;
    ...
    methods: {
      ...
      showMsgBoxTwo(id, res_name) {
        ...
        this.$bvModal
          ...
          .then(value => {
            if (value == true) {
              this.$http.get('/admin/delete_plan/' + id)
                .then(res => {
      ...
      changedefault(id) {
        this.$http.get('/admin/plan_status/' + id)
          .then(res => {
            if (res.data.status == true) {
              this.$toast({
                component: ToastificationContent,
