...
export default {
    ...
    methods: {
        ...
        cropImage() {
            ...
            this.category.image = this.$refs.cropper
                .getCroppedCanvas()
                .toDataURL();
        ...
        category_add() {
            ...
            data.append(
                ...
                this.category.secondary_categoryname
                    ? this.category.secondary_categoryname
                    : ""
            );
            data.append(
                "business_id",
                this.category.business_id ? this.category.business_id : ""
            );
            data.append(
                "status",
                this.category.status ? this.category.status : 0
            );
            data.append(
                "image",
    ...
    created() {
        this.$http.get("/admin/business_lists").then((res) => {
            this.options = res.data;
            this.Loading = false;
        });
        this.$http.get("/admin/get_lang").then((res) => {
            this.setting = res.data;
        });

        if (this.$route.params.id) {
            this.$http
                .get("/admin/edit_business_category/" + this.$route.params.id)
                .then((res) => {
                    console.log(res);
