        ...
        <b-card>
          <b-row>
            <b-col md="6">
              <div class="custom-search d-flex justify-content-start">
                <b-button variant="primary" :to="{ name: 'add_carrier' }">
                  <i class="fa-solid fa-plus"></i> {{ $t('Add Carrier') }}
                </b-button>
              </div>
          ...
          <vue-good-table :columns="columns" :line-numbers="true" :rows="rows" :rtl="direction" :search-options="{
            ...
            <template slot="table-row" slot-scope="props">
              ...
              <!-- Column: Name -->
              <div v-if="props.column.field === 'images'" class="text-nowrap">
              ...
              <span v-else-if="props.column.field === 'action'">
                <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                  cursor="pointer" @click="$router.push({ name: 'edit_carrier', params: { id: props.row.id } })" />
                <b-tooltip placement="left" :title="$t('Edit Carrier')" :target="`invoice-row-${props.row.id}-preview-icon`" />
                ...
                <b-tooltip placement="left" :title="$t('Delete Carrier')" :target="`invoice-row-delete-preview-icon`" />
    ...
    methods: {
      ...
      showMsgBoxTwo(id, res_name) {
        ...
        this.$bvModal
          ...
          .then(value => {
            if (value == true) {
              this.$http.get('/admin/delete_carrier/' + id)
                .then(res => {
      ...
      changedefault(id) {
        this.$http.get('/admin/carrier_status/' + id)
          .then(res => {
