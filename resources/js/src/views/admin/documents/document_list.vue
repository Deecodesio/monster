        ...
        <b-row>
          <b-col md="6">
            <div class="custom-search d-flex justify-content-start">
              <b-button variant="primary" :to="{ name: 'add_document' }">
                <i class="fa-solid fa-plus"></i>{{ $t('add') }} {{ $t('document') }}
              </b-button>
            </div>
  ...
  perPage: pageLength
          ...
          <b-badge :variant="statusVariant(props.row.status)">
            ...
            <span v-else-if="props.column.field === 'document_for'">
              <span v-if="props.row.document_for === '2'">

                {{ $t('restaurant') }}

              </span>
              <span v-else-if="props.row.document_for === '1'">
            ...
            <span v-else-if="props.column.field === 'action'">
              <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                cursor="pointer" @click="$router.push({ name: 'edit_document', params: { id: props.row.id } })" />
              <b-tooltip placement="left" :title="$t('edit') + ' ' + $t('document')"
...
export default {
  ...
  created() {
    ...
    this.$http.get('/admin/document_list')
      .then(res => {
        this.rows = res.data;
        this.Loading = false;
      })
  ...
  methods: {
    ...
    changedefault(id) {
      this.$http.get('/admin/document_status/' + id)
        .then(res => {
