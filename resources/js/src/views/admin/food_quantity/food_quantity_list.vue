        ...
        <b-row>
          <b-col md="6">
            <div class="custom-search d-flex justify-content-start">
              <b-button variant="primary" :to="{ name: 'add_food_quantity' }">
                <i class="fa-solid fa-plus"></i>{{ $t('add') }} {{ $t('Size') }}
              </b-button>
            </div>
          ...
          <b-badge :variant="statusVariant(props.row.status)">
            ...
            <span v-else-if="props.column.field === 'action'">
              <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                cursor="pointer" @click="$router.push({ name: 'edit_food_quantity', params: { id: props.row.id } })" />
...
import {
  BImg, BMedia, BAvatar, BBadge, BPagination, BFormGroup, BFormInput, BFormSelect, BDropdown, BDropdownItem, BCard, BButton,
} from 'bootstrap-vue'
...
export default {
  ...
  created() {
    ...
    this.$http.get('/admin/food-quantity-list')
      .then(res => {
        this.rows = res.data;
        this.Loading = false;
      })
