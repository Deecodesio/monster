      ...
      <b-tab @click="onTabChange(2)">
        ...
        <b-col md="12">
          <b-form-group :label="$t('Store')">
            <v-select v-model="brand.restaurant_id" :options="rows" label="restaurant_name" :reduce="sel => sel.id"
              :placeholder="$t('Select Store')" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" />
        ...
        <b-col md="12" id="primary" style="display: block;">
          <b-form-group :label="$t('name')">
            <b-form-input id="name" :placeholder="$t('name')" v-model="brand.name" required/>
          </b-form-group>
        </b-col>
        <b-col md="12" id="secondary" style="display: none;">
          <b-form-group :label="$t('name')">
            <b-form-input id="name" :placeholder="$t('name')" v-model="brand.secondary_name" />
          </b-form-group>
        </b-col>
        ...
        <b-col md="12">
          <b-form-group :label="$t('price')">
            <b-form-input id="name" :placeholder="$t('price')" v-model="brand.price" />
          </b-form-group>
        </b-col>
...
import {
  BRow, BCol, BFormGroup, BFormInput, BFormCheckbox, BForm, BButton, BFormSelect,
   BCardText, BCard,BTab, BTabs,  BAlert, BLink,
  BFormFile, BTooltip,
} from 'bootstrap-vue'
...
export default {
  ...
  methods: {
    ...
    addBrand() {
      ...
      this.$http
        .post("/admin/store_addons", brand2)
        .then(
          (response => {
  ...
  created() {
    ...
    if (this.$route.params.id) {
      this.$http.get('/admin/edit_addons/' + this.$route.params.id)
        .then(res => {
