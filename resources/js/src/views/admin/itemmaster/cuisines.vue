<template>
 ...
 <div class="custom-search d-flex justify-content-start">
        <b-button
            ...
            :to="{ name: 'add_cuisine'}"
          >
            <i class="fa-solid fa-plus"></i>{{$t('add')}}  {{$t('Cuisines')}} 
          </b-button>
    </div>
      ...
      >
        ...
        <span v-else-if="props.column.field === 'action'">
           <feather-icon
            ...
            class="mx-1"
            cursor="pointer"
            @click="$router.push({ name: 'edit_cuisine', params: { id: props.row.id }})"
          />
          <b-tooltip
          placement="left"
            :title="$t('edit') + ' ' + $t('Cuisines')"
...
export default {
  ...
  showMsgBoxTwo(id,res_name) {
      ...
      this.$bvModal
        ...
        .then(value => {
          if(value == true){
                      this.$http.post('/admin/delete_cuisine/'+id)
              .then(res => {    
