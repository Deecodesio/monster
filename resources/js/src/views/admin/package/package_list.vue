<template>
          ...
          <b-row>
            <b-col md="6">
              <div class="custom-search d-flex justify-content-start">
                <b-button variant="primary" :to="{ name: 'add_package' }">
                  <i class="fa-solid fa-plus"></i> {{ $t('Add Package') }}
                </b-button>
              </div>
            ...
            <template slot="table-row" slot-scope="props">
              ...
              <span v-else-if="props.column.field === 'action'">
                <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                  cursor="pointer" @click="$router.push({ name: 'edit_package', params: { id: props.row.id } })" />
                <b-tooltip placement="left" :title="$t('Edit Package')" :target="`invoice-row-${props.row.id}-preview-icon`" />
  
                <feather-icon :id="`invoice-row-delete-${props.row.id}-preview-icon`" icon="Trash2Icon" size="16" class="mx-1"
                  cursor="pointer" @click="showMsgBoxTwo(props.row.id, props.row.name)" />
                <b-tooltip placement="left" :title="$t('Delete Pacakge')" :target="`invoice-row-delete-${props.row.id}-preview-icon`" />
  ...
  export default {
    ...
    created() {
      ...
      this.$http.get('/admin/pacakge_list')
        .then(res => {
          this.rows = res.data;
          this.Loading = false;
    ...
    methods: {
      ...
      showMsgBoxTwo(id, res_name) {
        ...
        this.$bvModal
          ...
          .then(value => {
            if (value == true) {
              this.$http.get('/admin/delete_package/' + id)
                .then(res => {
      ...
      changedefault(id) {
        this.$http.get('/admin/pacakge_status/' + id)
          .then(res => {
