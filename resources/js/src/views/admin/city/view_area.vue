          ...
          <b-badge :variant="statusVariant(props.row.status)">
            ...
            <span v-else-if="props.column.field === 'action'">
              ...
              <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                cursor="pointer" @click="$router.push({ name: 'edit_area', params: { id: props.row.id } })" />
              <b-tooltip placement="left" :title="$t('edit city')" :target="`invoice-row-${props.row.id}-preview-icon`" />
              <feather-icon placement="left" :id="`invoice-row-delete-preview-icon`" icon="Trash2Icon" size="16" class="mx-1"
                cursor="pointer" @click="showMsgBoxTwo(props.row.id, props.row.area)" />
              <b-tooltip :title="$t('delete city')" :target="`invoice-row-delete-preview-icon`" />
...
export default {
  ...
  created() {
    ...
    this.$http.get('/admin/view_areas/' + this.$route.params.id)
      .then(res => {
  ...
  methods: {
    ...
    showMsgBoxTwo(id, res_name) {
      ...
      this.$bvModal
        ...
        .then(value => {
          if (value == true) {
            this.$http.post('/admin/delete_area_list/' + id)
              .then(res => {
    ...
    changedefault(id) {
      this.$http.get('/admin/area_status/' + id)
        .then(res => {
