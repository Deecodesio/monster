        ...
        <b-row>
          <b-col md="6">
            <div class="custom-search d-flex justify-content-start">
              <b-button variant="primary" :to="{ name: 'add_country' }">
                <i class="fa-solid fa-plus"></i> {{ $t('add country') }}
              </b-button>
            </div>
          ...
          <template slot="table-row" slot-scope="props">
            ...
            <span v-else-if="props.column.field === 'action'">
              <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                cursor="pointer" @click="$router.push({ name: 'add_country', params: { id: props.row.id } })" />
              <b-tooltip placement="left" :title="$t('edit_country')" :target="`invoice-row-${props.row.id}-preview-icon`" />

              <b-button v-if="props.row.is_default != 1" type="submit" variant="outline-warning" class="mr-1"
                @click="changedefault(props.row.id)">
                {{ $t('make default') }}
              </b-button>
              <b-button v-if="props.row.is_default != 0" type="submit" variant="outline-success" class="mr-1" disabled>
                {{ $t('default') }}
              </b-button>
            </span>
...
export default {
  ...
  methods: {
    changedefault(id) {
      this.$http.get('/admin/default_country/' + id)
        .then(res => {
          this.$toast({
