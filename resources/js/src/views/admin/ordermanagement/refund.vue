<template>
  ...
  perPage: pageLength
            ...
            <div v-if="props.column.field === 'user_names'" class="text-nowrap">
              ...
              <span class="text-nowrap" v-if="props.row.device_type === 'android'"><i
                  :id="'andriod' + props.row.restaurant_id" class="fa-brands fa-android" aria-hidden="true"></i> {{
                    props.row.user_name }}</span>
              <span class="text-nowrap" v-else-if="props.row.device_type === 'ios'"><i :id="'ios' + props.row.restaurant_id"
        ...
        >
         <span v-if="props.row.delivery_type === 1">
          ...
          <span v-else>
            ...
            <div v-if="props.column.field === 'order_id'" class="text-center">
              ...
              <br>
              <b-tooltip v-b-tooltip.hover.v-warning title="Delivery" :target="'delivery' + props.row.restaurant_id" />
              <b-tooltip v-b-tooltip.hover.v-warning title="Takeway" :target="'takeway' + props.row.restaurant_id" />
            </div>
            ...
            {{ props.row.is_cod }}
              ...
              <span v-if="props.row.status === 12 || props.row.status === 14">
                <b-badge :variant="statusVariant(1)">
                  Paid
                </b-badge>
              </span>
              <span v-else>
            ...
            <span v-else-if="props.column.field === 'view'">
              <feather-icon :id="`invoice-row-${props.row.restaurant_id}-preview-icon`" icon="EyeIcon" size="16"
                ...
                cursor="pointer" />
              <b-tooltip :title="$t('view') + ' ' + $t('order')"
                :target="`invoice-row-${props.row.restaurant_id}-preview-icon`" placement="left" />
            </span>
            ...
            <span v-else-if="props.column.field === 'Action'">
              ...
              <b-tooltip v-b-tooltip.hover.v-warning :title="'Login to ' + props.row.restaurant_name"
                :target="`invoice1-row-${props.row.restaurant_id}-preview-icon`" placement="left" />
            </span>
            <!-- Column: Action -->
            <span v-else-if="props.column.field === 'action'">
              <span>
                <b-dropdown variant="link" toggle-class="text-decoration-none" no-caret>
                  ...
                  <b-dropdown-item @click="sendInfo(props.row.id)" v-if="props.row.status === 13">

                    <a> <i class="fa-solid fa-share"></i> {{ $t('pay') }}</a>
                  </b-dropdown-item>
    ...
    <b-modal ref="my-modal" id="modal-login" cancel-variant="outline-secondary" ok-title="Pay Now" cancel-title="Close"
      centered :title="$t('make payment')" @show="resetModal" @hidden="resetModal" @ok="reason_add">
      <b-card-text>
      ...
      <form ref="form" @submit.prevent="reason_add">
        <b-form-group>
          ...
          <b-form-input id="email" type="text" placeholder="Total" v-model="banner.amount" />
        </b-form-group>
        <b-form-group>
          <label for="password">{{ $t('notes') }}</label>
          <b-form-input type="text" placeholder="Notes" v-model="banner.notes" />
        </b-form-group>
        <b-col>
          <!-- <b-button
...
import {
  BImg, BMedia, BModal, VBModal, BAlert, BAvatar, BBadge, BPagination, BFormGroup, BFormInput, BFormSelect, BDropdown, BDropdownItem, BButton, BCard, BTooltip,
} from 'bootstrap-vue'
...
export default {
  ...
  methods: {
        ...
        .then(
          (response => {
            ...
            if (response.data.status == true) {
              ...
              this.topay = response.data.amount;
    ...
    reason_add() {
      ...
      this.$http
        .post("/admin/add_userpayout", this.banner)
        .then(
          (response => {
            console.log(response);
            if (response.data.status == true) {
