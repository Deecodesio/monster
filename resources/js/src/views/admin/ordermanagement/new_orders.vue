<template>
  <b-row>
         ...
         <span v-if="props.row.delivery_type === 1">
          ...
          <span v-else>
            ...
            <div v-if="props.column.field === 'order_id'" class="text-nowrap">
              <span class="text-nowrap font-weight-bold" v-if="props.row.delivery_type === 1">
                <i :id="'delivery' + props.row.restaurant_id" class="fa fa-motorcycle" aria-hidden="true"></i>
                {{ props.row.order_id }}
              </span>
              <span class="text-nowrap font-weight-bold" v-else>
                <i :id="'takeway' + props.row.restaurant_id" class="fa-solid fa-person-walking" aria-hidden="true"></i>
                {{ props.row.order_id }}
              </span>
              <br>
            ...
            <span v-else-if="props.column.field === 'action'">
              <span>
                <b-dropdown variant="link" toggle-class="text-decoration-none" no-caret>
                  ...
                  <b-dropdown-item @click="make(props.row.id)" v-if="props.row.is_cod === 1">

                    <a><i class="fa-solid fa-share"></i> {{ $t('cancelled') }} </a>
                  </b-dropdown-item>
                  <b-dropdown-item @click="sendInfo(props.row.id)" v-else>

                    <a v-b-modal.modal-1><i class="fa-solid fa-share"></i> {{ $t('cancelled') }} </a>
                  </b-dropdown-item>
    ...
    <b-modal id="modal-1" :data-id="this.to_id" ref="my-modal" cancel-variant="outline-secondary" cancel-title="Close"
      ...
      <a class="btn btn-success" @click="refund(to_id)">{{ $t('refund') }} {{ $t('to') }} {{ $t('source_account') }}</a>
    </b-modal>
  </b-row>
</template>
...
export default {
  ...
  fetchOrders(); 
  window.setInterval(fetchOrders, 10 * 1000);  
