...
export default {
    ...
    computed: {
        ...
        filteredRows() {
            if (!this.shopFilter.district) {
                return this.rows;
            }
            return this.rows.filter(
                (row) =>
                    parseInt(row.city) === parseInt(this.shopFilter.district)
            );
        },
    },
    created() {
        var ud = localStorage.admin_email;
        this.$http.get("/admin/restaurant_list/" + ud).then((res) => {
            this.rows = res.data;
    ...
    methods: {
        ...
        changedefault(id) {
            this.$http.get("/admin/restaurant_status/" + id).then((res) => {
        ...
        showMsgBoxTwo(id, res_name) {
            ...
            this.$bvModal
                ...
                .then((value) => {
                    if (value == true) {
                        this.$http
                            .post("/admin/delete_restaurant", this.driver)
                            .then((res) => {
                                if (res.data.status == true) {
                                    var ud = localStorage.admin_email;
                                    this.$http
                                        .get("/admin/restaurant_list/" + ud)
                                        .then((res) => {
                                            this.rows = res.data;
                                        });
                                    this.$toast({
