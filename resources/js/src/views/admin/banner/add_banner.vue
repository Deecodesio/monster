                    ...
                    <div>
                        <b-card-text v-for="(img, index) in imgs" :key="index">
                            <b-img
                                ...
                                alt="banner photo"
                                @click="myMethod(img.banner_image)"
                                id="lazy"
                            />
                        </b-card-text>
                    </div>
...
export default {
    ...
    created() {
        this.$http.get("/admin/res_list").then((res) => {
            this.rows = res.data;
        });
        this.$http.get("/admin/banner_imgs").then((res) => {
            this.imgs = res.data;
        });
        this.$http.get("/admin/city_list").then((res) => {
            this.cities = res.data;
        });
        this.$http.get("/admin/business_category_lists").then((res) => {
            this.options_category = res.data;
        });
        this.$http.get("/admin/get_products").then((res) => {
            this.options_product = res.data.products;
        });
        if (this.$route.params.id) {
            this.$http
                .get("/admin/edit_banner/" + this.$route.params.id)
                .then((res) => {
    ...
    methods: {
        ...
        cropImage() {
            this.url1 = this.$refs.cropper
                .getCroppedCanvas({
                    width: 1200,
                    height: 628,
                })
                .toDataURL();
            this.banner.image = this.$refs.cropper
                .getCroppedCanvas()
                .toDataURL();
        ...
        add_banner() {
            ...
            data.append("banner_type", this.banner.banner_type);
