        ...
        <b-row>
          <b-col md="6">
            <div class="custom-search d-flex justify-content-start">
              <b-button variant="primary" :to="{ name: 'add_banner' }">
                <i class="fa-solid fa-plus"></i>{{ $t('add_banner') }}
              </b-button>
            </div>
  ...
  perPage: pageLength
          ...
          <template slot="table-row" slot-scope="props">
            ...
            <span v-else-if="props.column.field === 'featured'">
              <b-button :id="'featured' + props.row.banner_id"
                v-if="props.row.featured == 0" type="submit" variant="outline-warning"
                class="mr-1" @click="changefeatured(props.row.banner_id)">
                {{ $t('Not Featured') }}
              </b-button>
              <b-button :id="'nfeatured' + props.row.banner_id"
                v-if="props.row.featured == 1" type="submit" variant="outline-success"
                class="mr-1" @click="changefeatured(props.row.banner_id)">
                {{ $t(' Featured') }}
              </b-button>
              <b-tooltip placement="left" :title="$t('Remove From Featured')"
                :target="'nfeatured' + props.row.banner_id" v-if="props.row.featured == 1"/>
              <b-tooltip placement="left" :title="$t('Set as Featured')"
                :target="'featured' + props.row.banner_id" v-if="props.row.featured == 0"/>
            ...
            <span v-else-if="props.column.field === 'action'">
              ...
              <b-tooltip placement="left" :title="$t('edit_banner')" :target="`invoice-row-${props.row.id}-preview-icon`" />
              ...
              <b-tooltip placement="left" :title="$t('delete_banner')" :target="`invoice-row-delete-preview-icon`" />
            </span>
...
export default {
  ...
  methods: {
    ...
    changedefault(id) {
      this.$http.get('/admin/banner_status/' + id)
        .then(res => {
    ...
    changefeatured(id){
      this.$http.get('/admin/set_featured/'+ id)
        .then(res => {
          if (res.data.status == true) {
            this.$toast({
              component: ToastificationContent,
