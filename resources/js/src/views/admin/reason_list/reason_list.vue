<template>
        ...
        <b-row>
          <b-col md="6">
            <div class="custom-search d-flex justify-content-start">
              <b-button variant="primary" :to="{ name: 'add_reason' }">
                <i class="fa-solid fa-plus"></i> {{ $t('add') }} {{ $t('reason') }}
              </b-button>
            </div>
  ...
  perPage: pageLength
          ...
          <b-badge :variant="statusVariant(props.row.status)">
            ...
            <span v-else-if="props.column.field === 'cancellation_for'">
              <span v-if="props.row.cancellation_for === '1'">

                {{ $t('restaurant') }}

              </span>
              <span v-else-if="props.row.cancellation_for === '2'">
            ...
            <span v-else-if="props.column.field === 'action'">
              <feather-icon :id="`invoice-row-${props.row.id}-preview-icon`" icon="EditIcon" size="16" class="mx-1"
                cursor="pointer" @click="$router.push({ name: 'edit_reason', params: { id: props.row.id } })" />
              <b-tooltip :title="$t('edit') + ' ' + $t('reason')" :target="`invoice-row-${props.row.id}-preview-icon`" placement="left"/>

              <feather-icon :id="`invoice-row-delete-preview-icon`" icon="Trash2Icon" size="16" class="mx-1"
                cursor="pointer" @click="showMsgBoxTwo(props.row.id, props.row.reason)" />
              <b-tooltip :title="$t('delete') + ' ' + $t('reason')" :target="`invoice-row-delete-preview-icon`" placement="left"/>
            </span>
...
export default {
  ...
  created() {
    ...
    this.$http.get('/admin/reason_list')
      .then(res => {
        this.rows = res.data;
        this.Loading = false;
  ...
  methods: {
    ...
    showMsgBoxTwo(id, res_name) {
      ...
      this.$bvModal
        ...
        .then(value => {
          if (value == true) {
            this.$http.get('/admin/delete_reason/' + id)
              .then(res => {
    ...
    changedefault(id) {
      this.$http.get('/admin/reason_status/' + id)
        .then(res => {
