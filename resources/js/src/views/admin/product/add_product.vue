...
export default {
    ...
    computed: {
        allSelectedDistricts() {
            return this.product.flatMap((p) => p.district || []);
        },
    },
    mounted() {
        this.initTrHeight();
    ...
    methods: {
        ...
        getAvailableDistricts(product) {
            return product.city_list.filter((dis) => {
                ...
                // - It is NOT selected in other products OR
        ...
        cropImage() {
            var ind = this.productImage.length; // this.product.image.length
            this.url1.push(
                this.$refs.cropper
                    .getCroppedCanvas({
                        width: 1200,
        ...
        addProduct() {
            this.$refs.simpleRules.validate().then((success) => {
                if (success) {
                    let productCopy = this.product.map((item) => {
                    ...
                    product1.append(
                        "menu",
                        this.product.menu ? this.product.menu : ""
                    );
        ...
        async onStateChange(index) {
            const selectedStateId = this.product[index].state;
            this.$http
                ...
                .then((res) => {
                    ...
                    this.product[index].city_list = res.data;
                    this.product[index].district = [];
                });
        },
