    ...
    <b-row>
      ...
      <b-col md="3">
        <div style="position: relative;float: right;margin-left: 8%;" slot="button-next" class="swiper-button-next"
          id="swiper-button-next-brand"></div>
        <div style="position: relative;float: right;" slot="button-prev" class="swiper-button-prev"
          id="swiper-button-prev-brand"></div>

      </b-col>
    </b-row>

    <swiper class="swiper-multiple brand-pad" :options="swiperOptions"
      :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'">
      <swiper-slide v-for="(data, index) in rows" :key="index">
        <b>
          <b-img class=" rounded3 shadow-sm " :src="data.img" fluid
            @click="navigate(data.id, data.str, data.count, data.restaurant, data.restaurant_name)" :alt="data.str"
