<template>
  <div class="pt-1" v-if="!isempty">
    <b-row>
      ...
      <b-col cols="6">
        <h2>{{ $t("More") }} {{ $t("Stores") }}</h2>
        <hr>
      </b-col>
      <b-col cols="6" class="text-r">
        <b-link @click="$router.push({ name: 'more_restaurant', query: { name: 'morestore' } })">{{ $t("See") }}
          {{ $t("More") }}</b-link>
    ...
    <b-row>
      ...
      <b-col xxl="3" lg="3" md="6" cols="12" v-for="(data, index) in rows" :key="index" class="col-xxl-3 hover"
        ...
        <div :class=data.class class="rounded2 card">
          <b-img :class=data.class class="rounded2-1 imght2 round" :src="data.img" fluid :alt="data.text" width="425"
            height="200" />
...
export default {
  ...
  created() {
    ...
    this.isempty = true
    this.$http.get('/get_more_restaurant/' + lat + '/' + lng)
      .then(res => {
